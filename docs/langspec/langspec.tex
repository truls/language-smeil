...
\item Square brackets, e.g. \verb![ el ]!, denotes a list containing zero or
  ...
  successfully deserialized, the resulting SMEIL AST may not represent valid
  SMEIL. The BNF description of the corresponding SMEIL construct specifies when
  an empty list is allowed and when an object must occur at least once. The
  exception to this is lists containing exactly two elements. These corresponds
  to tuples.
...
\begin{grammar}
  ...
  <design-unit> ::= \{ <import-stm> \} <unit-component> \\ \{ <unit-component> \}

  <unit-component> ::= <network>
  \alt <process>
\end{grammar}
...
\begin{grammar}
  ...
  <network> ::= `network' <ident> `{' <network-decl> `}'
...
\begin{grammar}
  ...
  % Re-introduce named instances if needed
  %<instance> ::= `instance' <ident> `of' <ident> `(' [ <param-map> { `,' <param-map> } ]`)' `;'

  <instance> ::= `instance' <ident> \\ `(' [ <param-map> { `,' <param-map> } ] `)' `;'
...
\label{hsast}
\inputminted[fontsize=\small]{haskell}{../smeast/src/Language/SMEIL/Syntax.hs}
\end{document}
